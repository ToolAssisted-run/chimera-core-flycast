#!/bin/sh
# Applies the numbered patches to the extern/flycast submodule. Idempotent: a tree
...
root="$(cd "$here/.." && pwd)"
# FLYCAST_TREE points it at another checkout: how this script is tested without
# touching the tree a build depends on
tree="${FLYCAST_TREE:-$root/extern/flycast}"

if [ "$(git -C "$tree" rev-parse --show-toplevel 2>/dev/null)" != "$(cd "$tree" 2>/dev/null && pwd -P)" ]; then
	echo "extern/flycast is not checked out; run:" >&2
	echo "  git -C $root submodule update --init --recursive extern/flycast" >&2
	exit 1
fi
...
if [ "$wrong" -ne 0 ]; then
	echo "extern/flycast is partly patched. To start again from the submodule's HEAD:" >&2
	echo "  git -C extern/flycast reset --hard && git -C extern/flycast clean -fd && waterbox/apply-patches.sh" >&2
