...
/* chimera: the ONE change to this file.
 *
 * refsw was written to read its parameters out of video memory, because that is
 * where the hardware's TA puts them. Flycast's TA is high level: it parses the
 * TA FIFO into its own vertex lists and never writes CORE parameter blocks to
 * VRAM, so there is nothing here to decode. A host that has already decoded a
 * triangle registers it under its tag and answers here instead.
 *
 * Weakly declared: a build with no host (libswirl's, or a future one fed by a
 * low-level TA) keeps reading VRAM exactly as before.
 */
...
{
    FpuEntry entry = {};
    if (chimera_fpu_entry(core_tag.full, rect, &entry))
        return entry;
    Vertex vtx[3];
