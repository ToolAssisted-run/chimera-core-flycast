#!/bin/sh
# Builds the Flycast waterbox core package and installs it into a chimera
# checkout as build/Cores/flycast.chimeraCore.
#
...
flycast_pin="$(git -C "$root/extern/flycast" describe --tags --always 2>/dev/null || echo unknown)"
...
json.dump({
    ...
    "upstream": {"name": "Flycast", "pin": "$flycast_pin"},
    "builtOn": "$os_id",
...
mkdir -p "$cores_dir"
zip_path="$cores_dir/flycast.chimeraCore"
rm -f "$zip_path"
...
for cache in "$chimera_root"/build/CoreCache/flycast-*; do
